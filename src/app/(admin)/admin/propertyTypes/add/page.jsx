...
import { useAddPropertyType } from '@/hooks/usePropertyTypes';
...
function page() {
  ...
  const { isLoading, mutateAsync } = useAddPropertyType();
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const { message } = await mutateAsync({ ...propertyType });
      ...
      <div className="max-w-sm mb-10">
        <form className="space-y-4" onSubmit={handleSubmit}>
          ...
          <TextField
            name="englishTitle"
            label="عنوان انگلیسی"
            value={propertyType.englishTitle || ''}
            onChange={handleChange}
          />
